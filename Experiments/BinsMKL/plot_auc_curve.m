...
function plot_auc_curve()
  ...
  for i=1:size(xFilenameList,1)
    ...
    [X] = dlmread(sprintf('../ResultsMKL/perf_pn_%s_%s',regexprep(xFilename,'.*/',''),regexprep(yFilename,'.*/','')));
    plot(X(:,1),X(:,2));
    hold on;
  end
  ...
  for i=1:size(xFilenameList,1)
    xFilename = xFilenameList{i};
    yFilename = yFilenameList{i};
    [X] = dlmread(sprintf('../ResultsMKL/perf_pr_%s_%s',regexprep(xFilename,'.*/',''),regexprep(yFilename,'.*/','')));
    plot(X(:,1),X(:,2));
    hold on;
  end
  lgd = legend(xFilenameList);

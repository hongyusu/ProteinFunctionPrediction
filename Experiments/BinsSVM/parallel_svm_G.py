...
def singleJob(node, job):
  ...
  (paramInd,xFilename,yFilename,labelIndex,foldIndex,svmC,outputFilename,isTest,svmG) = job_detail
  try:
  ...
  for xFilename,yFilename,labelIndex,foldIndex in list(itertools.product(xFilenameList,yFilenameList,labelIndexList,foldIndexList)):
    for line in open('parameter_setting'):
      ...
      if words[0] == xFilename and words[1] == yFilename:
        svmC = words[2]
    for line in open('parameter_setting_G'):
      words = line.strip().split(' ')
      if words[0] == xFilename and words[1] == yFilename:
        svmG = words[2]
    ...
    job_queue.put( (paramInd,(str(paramInd),xFilename,yFilename,str(labelIndex),str(foldIndex),svmC,outputFilename,isTest,svmG)) )

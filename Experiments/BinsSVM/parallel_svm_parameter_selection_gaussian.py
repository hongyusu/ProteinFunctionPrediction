...
def singleJob(node, job):
  ...
  (paramInd,xFilename,yFilename,labelIndex,foldIndex,svmC,svmG,outputFilename,isTest) = job_detail
  try:
    if checkfile(outputFilename):
      ...
      logging.info('\t--> (priority) %d (node)%s (filename) %s' %(priority, node, outputFilename))
      os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd /cs/work/group/urenzyme/workspace/ProteinFunctionPrediction/Experiments/BinsSVM/; nohup matlab -nodisplay -nosplash -r "single_SVM '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'" > /var/tmp/tmpsu'  """ % (node,xFilename,yFilename,labelIndex,foldIndex,svmC,outputFilename,isTest,svmG) )
      logging.info('\t--| (priority) %d (node)%s (filename) %s' %(priority, node, outputFilename))
      fail_penalty = -1
      time.sleep(1)
...
def run():
  ...
  gList    = ['0.01','0.5','0.1','1','10']
  # generate job queue, will iterate over c,k,label
  for xFilename,yFilename,labelIndex,foldIndex,svmG in list(itertools.product(xFilenameList,yFilenameList,labelIndexList,foldIndexList,gList)):
    # read svm C
    ...
    job_queue.put( (paramInd,(str(paramInd),xFilename,yFilename,str(labelIndex),str(foldIndex),svmC,svmG,outputFilename,isTest)) )
